...
(function() {
  ...
  function initAuthenticatedApp() {
    ...
    // Sync display name from server if authenticated
    if (auth.authenticated && auth.user) {
      if (auth.user.name) {
        state.username = auth.user.name;
        storageSet(STORAGE_KEYS.USERNAME, auth.user.name);
        if (elements.profileNameInput) elements.profileNameInput.value = auth.user.name;
      }
      if (auth.user.emoji) {
        state.emoji = auth.user.emoji;
        storageSet(STORAGE_KEYS.EMOJI, auth.user.emoji);
        if (elements.profileEmojiBtn) elements.profileEmojiBtn.textContent = auth.user.emoji;
      }
    }
  }
  ...
  function saveProfile() {
    ...
    // Persist to server for OAuth users
  ...
  function setupProfilePage() {
    ...
    if (backBtn) {
      backBtn.addEventListener('click', () => showView('landing'));
    }
    ...
    if (emojiBtn && emojiPicker) {
      ...
      emojiPicker.addEventListener('click', (e) => {
        ...
        // Also update the in-lobby profile emoji button
    ...
    if (saveBtn && nameInput) {
      ...
      nameInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') saveProfilePage();
      });
    }
  ...
  async function saveProfilePage() {
    ...
    // Update in-lobby profile editor if present
    if (elements.profileNameInput) elements.profileNameInput.value = newName;

    // Save to server for OAuth users
    try {
      ...
      // Ignore server errors — local save is enough
    }
  ...
  async function loadProfilePage() {
    ...
    // Set current values from local state
    if (nameInput) nameInput.value = state.username;
    if (emojiBtn) emojiBtn.textContent = state.emoji;

    // Fetch profile from server (for OAuth display name and linked providers)
    try {
      ...
      if (res.ok) {
        ...
        renderProvidersList(providersList, []);
      }
    } catch {
      renderProvidersList(providersList, []);
    }
  ...
  function renderProvidersList(container, providers) {
    ...
    // Build linked providers map
    const linkedMap = {};
    providers.forEach(p => { linkedMap[p.provider] = p; });

    // Fetch available providers to know which can be linked
    fetch('/api/auth/providers')
      .then(r => r.json())
      .then(available => {
        let html = '';

        // Show linked providers first
        for (const p of providers) {
          const config = providerConfigs[p.provider];
          if (!config) continue;
          html += `
            <li class="profile-provider-item connected">
              <div class="profile-provider-info">
                ${config.icon}
                <div>
                  <span class="profile-provider-name">${config.name}</span>
                  <span class="profile-provider-email">${escapeHtml(p.email || p.name || 'Connected')}</span>
                </div>
              </div>
              <button class="btn btn-small btn-secondary profile-unlink-btn" data-provider="${p.provider}">Disconnect</button>
            </li>`;
        }

        // Show unlinked but available providers
        for (const [key, config] of Object.entries(providerConfigs)) {
          if (linkedMap[key] || !available[key]) continue;
          html += `
            <li class="profile-provider-item">
              <div class="profile-provider-info">
                ${config.icon}
                <div>
                  <span class="profile-provider-name">${config.name}</span>
                  <span class="profile-provider-email">Not connected</span>
                </div>
              </div>
              <a href="${config.linkUrl}" class="btn btn-small btn-primary profile-link-btn">Connect</a>
            </li>`;
        }

        if (!html) {
          html = '<li class="profile-provider-empty">No providers configured</li>';
        }
      ...
      .catch(() => {
        container.innerHTML = '<li class="profile-provider-empty">Failed to load providers</li>';
      });
  }
